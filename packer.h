...
public:
    struct Node
    {
        ...
        bool remove(const Rect& box)
        {
            ...
            if (used && box.x == r.x && box.y == r.y && box.w == r.w &&
                box.h == r.h) {
                used = false;
                // LOGD("Removing box");
                return true;
            ...
            if (r0 && r1) {
                if (kids[0]->used || kids[1]->used)
                    return false;
                // LOGD("Joining %d,%d %dx%d and %d,%d %dx%d", kids[0]->r.x,
                // kids[0]->r.y, kids[0]->r.w, kids[0]->r.h, kids[1]->r.x,
                // kids[1]->r.y, kids[1]->r.w, kids[1]->r.h);
                kids[0] = nullptr;
                kids[1] = nullptr;
                used = false;
                return true;
        ...
        bool add(Rect& box)
        {
            ...
            // LOGD("Trying (%d,%d) against this rect (%d,%d,%d,%d)", box.w,
            // box.h, r.x, r.y, r.w, r.h);
    ...
    bool add(Rect& r) override { return root.add(r); }

    void remove(const Rect& r) override { root.remove(r); }
